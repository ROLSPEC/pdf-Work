...
import { MagnifyingGlass, ArrowRight, Lightning, ShieldCheck, Heart } from "@phosphor-icons/react";
import { useNavigate } from "react-router-dom";
import { toast } from "sonner";

const suggestForFile = (file) => {
  const n = (file?.name || "").toLowerCase();
  if (n.endsWith(".pdf")) return ["merge", "compress", "protect", "pdf-to-word"];
    ...
    <div className="relative paper-grid">
          ...
          <div className="grid md:grid-cols-[1.2fr_1fr] gap-10 items-start">
            <div>
              ...
              <div className="grid grid-cols-3 gap-3 mt-10 max-w-lg">
                ...
                <MiniFeature icon={<Lightning size={18} weight="bold" />} label="24h auto-delete" />
          ...
          <div className="flex flex-col md:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Search — try 'compress' or 'protect'"
            ...
            <div className="flex gap-1.5 overflow-x-auto md:overflow-visible">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`whitespace-nowrap px-3 py-2 border-2 font-mono text-xs uppercase tracking-widest font-bold transition-colors ${cat === c.id ? "btn-ink border-border" : "bg-card border-border hover:bg-primary hover:text-primary-foreground"}`}
                >
                  {c.name}
                </button>
              ))}
      ...
      <section className="border-t-2 border-border bg-card">
        <div className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-3 gap-6">
          ...
          <WhyCard label="ephemeral" tone="bg-card border-border" title="Cloud files gone in 24 h." desc="Server jobs auto-expire. You can also nuke your history with one click any time." />
          <WhyCard label="cheap" tone="bg-accent/20" title="$1 lifetime. No trial games." desc="Every tool, forever. No monthly renewals. No upsell walls." />
        </div>
      </section>
